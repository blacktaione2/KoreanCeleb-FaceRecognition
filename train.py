...
PHASE1_CKPT = 'models/phase1_resnet.pt'

def train_model(alpha=1.6, beta=0.7, num_epochs=30, patience=8, min_smile_acc=0.70):
    ...
    if os.path.exists(PHASE1_CKPT):
        ...
        for epoch in range(PHASE1_EPOCHS):
            ...
            for images, celeb_labels, smile_labels in train_loader:
                images, celeb_labels, smile_labels = images.to(device), celeb_labels.to(device), smile_labels.to(device)
            ...
            with torch.no_grad():
                for images, celeb_labels, smile_labels in val_loader:
                    ...
                    loss = (alpha * criterion(c_preds, celeb_labels)) + (beta * criterion(s_preds, smile_labels))
    ...
    # ── 2단계: Backbone 해제, 전체 Fine-tuning ───────────────────────
...
if __name__ == "__main__":
    train_model(alpha=1.6, beta=0.7)