...
import torch
from train import train_model, PHASE1_CKPT
import os

# ── 실험 설정 ────────────────────────────────────────────────────────
# alphas = [0.5, 1.0, 1.5, 2.0]
# betas  = [0.5, 1.0, 1.5, 2.0]
alphas = [1.3, 1.4, 1.5, 1.6, 1.7]  # Alpha 1.5 중심 ±0.2
betas  = [0.7, 0.8, 0.9, 1.0, 1.1]  # Beta 1.0 중심, 실패 구간(1.5) 완전 제외
OUTPUT_CSV = 'grid_search_results.csv'
OUTPUT_IMG = 'grid_search_heatmap.png'
...
for ax, (col, title) in zip(axes, plot_configs):
    ...
    sns.heatmap(
        ...
        cmap='YlGnBu',
        vmin=0.0,
        vmax=None,   # [수정] 세분화 범위에선 0.0 없을 가능성 낮으므로 자동 범위
        ax=ax
    )
...
plt.suptitle('Grid Search (Refined): Alpha vs Beta', fontsize=15, y=1.02)
plt.tight_layout()
